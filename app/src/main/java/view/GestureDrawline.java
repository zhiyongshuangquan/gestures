...
public class GestureDrawline extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        paint.setColor(Color.parseColor("#FFFAFA"));// 设置默认连线颜色
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (currentPoint == null && pointAt == null) {
                    ...
                    if (currentPoint == null) {// 先判断当前的point是不是为null
                        ...
                        currentPoint
                                .setPointState(GesturePoint.POINT_STATE_SELECTED);
                        passWordSb.append(currentPoint.getNum());
                    }
                }
                if (pointAt == null
                        || currentPoint.equals(pointAt)
                        || GesturePoint.POINT_STATE_SELECTED == pointAt
                        .getPointState()) {
                    ...
                    if (betweenPoint != null
                            && GesturePoint.POINT_STATE_SELECTED != betweenPoint
                            .getPointState()) {
                        // 存在中间点并且没有被选中
    ...
    private void drawErrorPathTip() {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        paint.setColor(Color.parseColor("#DB1509"));// 设置默认线路颜色
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
